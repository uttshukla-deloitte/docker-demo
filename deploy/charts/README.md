# Helm Chart Templates for web server deployment in Kubernetes

This project provides template Helm Charts for deploying a simple web application into any Kubernetes based cloud.
...
| File                                              | Description                                                           |
|---------------------------------------------------|-----------------------------------------------------------------------|  
| `/deploy/charts/Chart.yaml`                    | The definition file for your application                           | 
| `/deploy/charts/values.yaml`                   | Configurable values that are inserted into the following template files      | 
| `/deploy/charts/templates/deployment.yaml`     | Template to configure your application deployment.                 | 
| `/deploy/charts/templates/service.yaml`        | Template to configure your application service deployment.                 | 
| `/deploy/charts/templates/secret.yaml`         | Template to configure your secrets for application deployment.                 | 
| `/deploy/charts//_helpers.tpl`                 | Helper to enable calling other functions        | 
...
1. You have a Kubernetes cluster  
  This could be one hosted by a cloud provider or running locally, for example using [k3d](https://k3d.io/v5.3.0/usage/commands/k3d_cluster_create/)
...
```sh
image:
  repository: <namespace>/webserver
```
...
* Remove the application with `helm delete --purge webserver`.
...
* Find the deployment using `helm list --all` and searching for an entry with the chart name "nodeserver".
* Remove the application with `helm uninstall webserver`.
